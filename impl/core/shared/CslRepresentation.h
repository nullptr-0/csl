...
namespace CSL {
    ...
    public:
        ...
        PrimitiveType(Primitive type, std::vector<std::pair<std::string, std::unique_ptr<Type::Type>>>&& allowedValues, std::vector<std::shared_ptr<Annotation>> annotations, FilePosition::Region region)
        ...
        const std::vector<std::pair<std::string, std::unique_ptr<Type::Type>>>& getAllowedValues() const { return allowedValues; }
    ...
    public:
        ...
        public:
            KeyDefinition(
                ...
                std::vector<std::shared_ptr<Annotation>> annotations,
                std::optional<std::pair<std::string, std::unique_ptr<Type::Type>>> defaultValue,
                FilePosition::Region nameRegion
            ) : name(std::move(name)),
                ...
                nameRegion(std::move(nameRegion)) {
            }
            const std::string& getName() const { return name; }
            bool getIsWildcard() const { return isWildcard; }
            bool getIsOptional() const { return isOptional; }
            const std::shared_ptr<CSLType>& getType() const { return type; }
            const std::vector<std::shared_ptr<Annotation>>& getAnnotations() const { return annotations; }
            const std::optional<std::pair<std::string, std::unique_ptr<Type::Type>>>& getDefaultValue() const { return defaultValue; }
            const FilePosition::Region& getNameRegion() const { return nameRegion; }
        ...
        TableType(
            std::vector<std::shared_ptr<KeyDefinition>> explicitKeys,
        ...
        const std::vector<std::shared_ptr<KeyDefinition>>& getExplicitKeys() const { return explicitKeys; }
    ...
    public:
        LiteralExpr(std::unique_ptr<Type::Type> type, std::string value, FilePosition::Region region)
            : Expr(Kind::Literal, region), type(std::move(type)), value(std::move(value)) {
        }

        const std::unique_ptr<Type::Type>& getType() const { return type; }
